...
fun LeerRangosFirebase(
    //ONSUCCES DEVUELVE LA CONFIGURACION DE RANGOS
    onSuccess: (ConfiguracionDeRangos) -> Unit,
    onError: (String) -> Unit
) {
    ...
    consulta.get()
        ...
        .addOnSuccessListener { lanzamiento ->
            ...
            if (lanzamiento.exists()) {
                ...
                val datosObtenidos = lanzamiento.getValue(ConfiguracionDeRangos::class.java)
                //SE COMPRUEBA QUE EL CARGO NO SEA NULO (NULL)
                if (datosObtenidos != null) {
                    Log.d("Rangos", "El rango obtenido minimo es ${datosObtenidos.rangoMinimo}, y el maximo es ${datosObtenidos.rangoMaximo}")
                    //PARA FINAMENTE DEVOLVER EL DATO
                    //MEDIANTE EL ONSUCCES A LA SCREEN DEL LOGIN
                    onSuccess(datosObtenidos)
                } else {
                    Log.d("Rangos", "No se puedo leer los datos de la base de datos")
                    //PARA FINAMENTE DEVOLVER EL DATO
                    //MEDIANTE EL ONSUCCES A LA SCREEN DEL LOGIN
                    onError("Error en la base da datos: No se pudieron leer los datos.")
                }
            } else {
                Log.d("Rangos", "No hay datos en la base de datos")
                //PARA FINAMENTE DEVOLVER EL DATO
                //MEDIANTE EL ONSUCCES A LA SCREEN DEL LOGIN
                onSuccess(ConfiguracionDeRangos(0.0, 0.0))
            }
        }
