...
fun LeerRangosAlertaFirebase(
    //ONSUCCES DEVUELVE LA CONFIGURACION DE RANGOS
    onSuccess: (ConfiguracionRangosAlertaDispositivo) -> Unit,
    onError: (String) -> Unit
) {
    ...
    consulta.get()
        ...
        .addOnSuccessListener { lanzamiento ->
            ...
            if (lanzamiento.exists()) {
                ...
                val datosObtenidos = lanzamiento.getValue(ConfiguracionRangosAlertaDispositivo::class.java)
                //SE COMPRUEBA QUE EL CARGO NO SEA NULO (NULL)
                if (datosObtenidos != null) {
                    Log.d("RangoAlertaEstado", "El estado obtenido es ${datosObtenidos.estadoDeActivacion}")
                    //PARA FINAMENTE DEVOLVER EL DATO
                    //MEDIANTE EL ONSUCCES A LA SCREEN DEL LOGIN
                    onSuccess(datosObtenidos)
                } else {
                    Log.d("RangoAlertaEstado", "No se puedo leer los datos de la base de datos")
                    //PARA FINAMENTE DEVOLVER EL DATO
                    //MEDIANTE EL ONSUCCES A LA SCREEN DEL LOGIN
                    onError("Error en la base da datos: No se pudieron leer los datos.")
                }
            } else {
                Log.d("RangoAlertaEstado", "No hay datos en la base de datos")
                //PARA FINAMENTE DEVOLVER EL DATO
                //MEDIANTE EL ONSUCCES A LA SCREEN DEL LOGIN
                onSuccess(ConfiguracionRangosAlertaDispositivo(false))
            }
        }
