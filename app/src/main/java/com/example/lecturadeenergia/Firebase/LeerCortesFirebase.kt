...
fun LeerCortesFirebase(
    //ONSUCCES DEVUELVE LA CONFIGURACION DE RANGOS
    onSuccess: (ConfiguracionDeCoriente) -> Unit,
    onError: (String) -> Unit
) {
    ...
    consulta.get()
        ...
        .addOnSuccessListener { lanzamiento ->
            ...
            if (lanzamiento.exists()) {
                ...
                val datosObtenidos = lanzamiento.getValue(ConfiguracionDeCoriente::class.java)
                //SE COMPRUEBA QUE EL CARGO NO SEA NULO (NULL)
                if (datosObtenidos != null) {
                    Log.d("Cortes", "El corte obtenido es ${datosObtenidos.apagadoEncendido}")
                    //PARA FINAMENTE DEVOLVER EL DATO
                    //MEDIANTE EL ONSUCCES A LA SCREEN DEL LOGIN
                    onSuccess(datosObtenidos)
                } else {
                    Log.d("Cortes", "No se puedo leer los datos de la base de datos")
                    //PARA FINAMENTE DEVOLVER EL DATO
                    //MEDIANTE EL ONSUCCES A LA SCREEN DEL LOGIN
                    onError("Error en la base da datos: No se pudieron leer los datos.")
                }
            } else {
                Log.d("Cortes", "No hay datos en la base de datos")
                //PARA FINAMENTE DEVOLVER EL DATO
                //MEDIANTE EL ONSUCCES A LA SCREEN DEL LOGIN
                onSuccess(ConfiguracionDeCoriente(false))
            }
        }
